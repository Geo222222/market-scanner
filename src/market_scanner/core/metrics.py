...
class SymbolSnapshot(BaseModel):
    ...
    exchange: str = Field(..., description="Exchange name - REQUIRED, never null/empty/defaulted.")
    ...
    z_15s: float = Field(0.0, description="Z-score of very short-term momentum (approximately 15 second horizon).")
    z_1m: float = Field(0.0, description="Z-score of 1 minute momentum window.")
    z_5m: float = Field(0.0, description="Z-score of 5 minute momentum window.")
    vwap_distance: float = Field(0.0, description="Normalized distance from VWAP for the latest trade.")
    rsi14: float = Field(50.0, description="Classic RSI(14) momentum indicator.")
