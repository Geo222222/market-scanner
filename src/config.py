...
class Settings(BaseSettings):
    ...
    postgres_url: Optional[str] = Field(
        default="postgresql+psycopg://nexus:nexus@postgres:5432/nexus",
    ...
    signal_channel: str = Field(default="nexus.signals", description="Redis pub/sub channel for signals.")

    # Trading Engine Settings
    max_position_size: float = Field(default=0.1, description="Maximum position size as fraction of portfolio.")
    max_drawdown: float = Field(default=0.05, description="Maximum allowed drawdown.")
    min_confidence: float = Field(default=70.0, description="Minimum signal confidence threshold.")
    trading_enabled: bool = Field(default=False, description="Enable automated trading.")
